...
Pin::Pin(char GPIOx, uint8_t pin, uint16_t fun) {
	...
	else RCC_AHB1PeriphClockCmd(RCC_AHB1Periph_GPIOI, ENABLE);
...
void Pin::pwmInit(int16_t period, uint32_t prescaler, uint8_t channel) {
  ...
  _tPWM.TIM_OCPolarity = TIM_OCPolarity_High; 
	...
	_tPWM.TIM_OCIdleState = TIM_OCIdleState_Set;
	_tPWM.TIM_OCNIdleState = TIM_OCNIdleState_Set;
	_channel = channel;
