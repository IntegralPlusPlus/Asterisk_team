...
void OpenMV::read() {
	...
	//                    crc8(data[0], data[1], data[2], data[3])
	if (gotData && crc == crc8(data, 4)) {
		//_angleBlue = adduct(2 * _angleBlue);
		//_angleYellow = adduct(2 * _angleYellow);;
		_distBlue = data[0];
...
/*
uint8_t OpenMV::crc8(uint8_t d1, uint8_t d2, uint8_t d3, uint8_t d4) {
