...
void ProcessingCoord::setParams(int16_t x, int16_t y, int16_t angle) {
	_x = x;
	_y = y;
	_angle = angle;
...
bool ProcessingCoord::myGoalLine(int16_t x, int16_t y) {
	return x >= -GOAL_OUT_X_THRESHOLD && x <= GOAL_OUT_X_THRESHOLD && y < GOAL_OUT_Y_THRESHOLD + DELTA_DIST;
}

bool ProcessingCoord::enemyGoalLine(int16_t x, int16_t y) {
	return x >= -GOAL_OUT_X_THRESHOLD && x <= GOAL_OUT_X_THRESHOLD && y > DIST_BETWEEN_GOALS - (GOAL_OUT_Y_THRESHOLD + DELTA_DIST);
}
